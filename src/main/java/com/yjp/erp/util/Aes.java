...
/**
 * @author wyf
 * @date 2019/4/2 下午 4:42
 **/
public class Aes {

    private static final int APIKEY_LENGTH = 16;

    private static final int HEX_NUM = 2;

    private static String handlerKey(String apiKey) {
        String tempkey = "HDALd)9dkA*&1kS$CKSJ}{|A";
        if (apiKey.length() > APIKEY_LENGTH) {
            apiKey = apiKey.substring(0, APIKEY_LENGTH);
        } else if (apiKey.length() < APIKEY_LENGTH) {
            apiKey = apiKey + tempkey.substring(0, APIKEY_LENGTH - apiKey.length());
        }
        return apiKey;
    }
    ...
    public static byte[] parseHexStr2Byte(String hexStr) {
        ...
        for (int i = 0;i< hexStr.length()/HEX_NUM; i++) {
